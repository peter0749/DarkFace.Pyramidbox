...
def preprocess(img, bbox_labels, mode, image_path):
    ...
    interp = np.random.choice(interp_mode, p=[0.1,0.2,0.35,0.1,0.25])
    try:
        img = cv2.resize(img, (cfg.resize_width, cfg.resize_height), interpolation=interp)
    except:
        warnings.warn('Potential empty images')
        img = np.zeros((cfg.resize_height, cfg.resize_width, 3), dtype=np.uint8)
