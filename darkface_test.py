...
def VOC2010_map(precision, recall):
    recall    = np.concatenate(([1.0], recall, [0.0]))
    precision = np.concatenate(([0.0], precision, [0.0]))
    for i in range(1, recall.shape[0]):
        precision[i] = max(precision[i], precision[i-1]) # fix: monotonic increasing
    i = np.where(recall[1:]!=recall[:-1])[0]
    return np.sum((recall[i]-recall[i+1]) * precision[i]), precision, recall
...
def get_gt():
    ...
    with open(cfg.FACE.TEST_FILE, 'r') as fp:
        for line in fp.readlines():
            ...
            bboxes = []
            for i in range(n):
                x,y,w,h = tuple(map(float, l[ i*5 + 2 : i*5 + 4 + 2])) # (path, N, x1,y1,w1,h1,s1, x2,y2,w2,h2,s2, ...)
                bboxes += [(x,y,x+w,y+h)]
            if path not in res:
                res[path] = bboxes
            else:
...
if __name__ == '__main__':
    ...
    for img_path in tqdm(list(gt), total=len(list(gt))):
        ...
        max_im_shrink = np.sqrt(
            1280 * 720 / (img.shape[0] * img.shape[1]))
            ...
            for bbox in dets:
                ...
                for i,bbox_y in enumerate(gt[img_path]):
                    ...
                    if iou>iou_max:
                        iou_max = iou
                        idx = i
                if iou_max<args.iou or hit[idx]: # mismatched
                    fps[th_n]+=1
                else:
                    hit[idx]=True
                    tps[th_n]+=1
    ...
    average_precision, precision, recall = VOC2010_map(precision, recall)
    ...
    plt.ylim([0.0, 1.05])
    plt.xlim([0.0, 1.0])
    plt.title('Precision-Recall curve: AP={:.2f}%'.format(average_precision*100.0))
